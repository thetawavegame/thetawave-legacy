...
use data_include::{ IncludeData, load_include_data, generate_configs };
...
fn main() -> amethyst::Result<()> {
    ...
    let items = <ItemsResource as Config>::load_bytes(items)
    ...
    let store = <StoreResource as Config>::load_bytes(store)
        .expect("failed to load data file: store.ron");
    let game_parameters =
        <GameParametersResource as Config>::load_bytes(game_parameters)
            .expect("failed to load data file: game_parameters.ron");
    let formations = <FormationsResource as Config>::load_bytes(formations)
