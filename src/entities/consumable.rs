...
use crate::{
    components::{ConsumablePool},
    resources::SpriteResource,
};


pub fn spawn_consumable(entities: &Entities, sprite_resource: &ReadExpect<SpriteResource>, consumable_pool: &mut ConsumablePool, spawn_position: Vector3<f32>, lazy_update: &ReadExpect<LazyUpdate>) {
    ...
    let random_consumable_name = consumable_pool.available_consumables.choose(&mut rand::thread_rng()).cloned().unwrap();
    let random_consumable = consumable_pool.consumables[&random_consumable_name].clone();
