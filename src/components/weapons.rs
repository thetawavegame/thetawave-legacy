...
/*
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct MissileLauncherComponent {
    pub offset: Vector2<f32>, // spawn position of blasts offset from center of entity
    pub damage: f32,
    pub size_multiplier: f32,
}

impl Component for MissileLauncherComponent {
    type Storage = DenseVecStorage<Self>;
}

impl MissileLauncherComponent {
    pub fn fire(
        &self,
        source_transform: &Transform,
        entities: &Entities,
        ...
        let missile_sprite_render = SpriteRender {
            sprite_sheet: sprite_resource.enemies_sprite_sheet.clone(),
            sprite_number: 15,
        };

        let missile_hitbox = Hitbox2DComponent {
            width: BLAST_HITBOX_DIAMETER * self.size_multiplier,
            height: BLAST_HITBOX_DIAMETER * self.size_multiplier,
            offset_x: 0.0,
            offset_y: 0.0,
